        ...
        <div className="grid lg:grid-cols-2 gap-10 items-start">
          ...
          <div className="space-y-5">
            ...
            {promo.expiresAt && countdown && (
              <div className={`flex items-center gap-2 text-sm font-semibold ${isExpired || countdown === 'Encerrada' ? 'text-red-600' : 'text-violet-700'}`}>
            ...
            {overflowQty > 0 && !isExpired && (
              <div className="bg-purple-50 border border-purple-200 rounded-lg p-3 text-sm space-y-1">
                <div className="flex items-center gap-1.5 font-semibold text-violet-800"><Info className="w-4 h-4" /> Resumo do pedido</div>
                {promoQty > 0 && <div className="text-violet-800">{promoQty}x a {formatPrice(promoPriceLocal, currency, true)} <span className="text-xs">(preço promo)</span></div>}
                <div className="text-violet-800">{overflowQty}x a {formatPrice(originalPriceLocal, currency, true)} <span className="text-xs">(preço original)</span></div>
                <div className="font-bold text-orange-800 border-t border-purple-200 pt-1">Total: {formatPrice(totalLocal, currency, true)}</div>
              </div>
            )}
